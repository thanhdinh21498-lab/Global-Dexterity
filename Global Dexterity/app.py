...
st.header("And I want to go around to take a look at other's projects, not just sit there and wait for you guys, so that's why you here!!!!")
...
Each line represents one question, and the x-axis shows where respondents grew up.
    ...
    chart = (
        ...
        .encode(
            x=alt.X(f"{COL_COUNTRY}:N", sort=None, axis=alt.Axis(labelAngle=0)),  # rotate labels
